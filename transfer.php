<?php
	...
	if(isset($_POST['submit']))
	{
		...
		if(!$money || !$account)
		{
			...
			$query="select idClient from Bank_Account";
			...
			foreach($row as $cvalue){
				if($cvalue==$account)
				{
					...
					$query=mysql_query("UPDATE Bank_Account SET Balance=Balance+".$money." WHERE idClient='".$account."')");
					$res=mysql_query($query);
					...
					$error_message="That Account doesn't exist!";
				}
			}
		}
